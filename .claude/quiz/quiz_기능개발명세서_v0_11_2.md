...
> **버전:** v0.14.1 · **상태:** 초안 — 본문 = 현행 **19유형**(계산 3 + 위앞옆 3 + G군 3 + A군 6 + H군 4) · **최종 수정일:** 2026-08-31
> **기준 마스터:** v1.9.4 (§5.2 클라↔서버 복본 동기화 · §6.7 정답 은닉 규약 · §1 연령 스테이지 · §6.5 요금제)
> ℹ 마스터 §9 인덱스의 이 문서 버전 표기는 오래 v0.11.2 에 머물러 있었고 **v1.9.4 에서 v0.14.1 로 동기화했다.** 다음에 이 문서를 올릴 때 §9 도 함께 올릴 것.
...
| 버전 | 날짜 | 핵심 |
|---|---|---|
| **v0.14.1** | **08-31** | **전 유형 재실측 — "문제가 성립 안 하고 난이도가 안 맞는다"(사용자)(Claude Code).** 19종 × 4등급 × 5스테이지를 **500문항씩(140조합)** 다시 쟀다. 정본 = `난이도_재설계 §11`, 요약·수정 목록 = 본 문서 **§5.4.0**. **종전 검증이 개편한 축만 봤다** — §8·§9 의 "등급 역전 0"은 개수 밴드·굴곡·가짓수에 대한 것이었고 **유형 전체를 본 것이 아니었다.** 드러난 것: ① **정상 경로에서 성립하지 않는 문항** — `A-b` 하 **14%** 가 정답 0칸(찍을 칸이 없어 `run.js:422` 가 제출을 막는다), `A-d` 하 **10.8%** 가 정답 0(`max===min` 인데 `which='diff'`). 원인은 같다 — `resolveCfg` 가 발자국·개수를 한 값으로 고정하는데 **`widenCfg` 를 G-c·H-a/H-b 에만** 걸어서, 나머지 루프는 같은 좁은 공간을 60번 훑는다. `gen.js:193-196` 주석이 스스로 진단해 놓고 적용을 빠뜨린 자리다. ② **찍으면 맞는 등급** — `A-f` 하가 **500/500 "2가지"**(정답 1종), `H-d` 최상이 상자 4×4×4 하나·정답 {8,24} 라 **10문항 세션의 100%에서 중복**. `boxTotal[64,100]` 인데 `cap=maxH=4` 라 5×4×4 를 못 쓴 결과다. ③ **등급 역전** — `H-c` 제시물 개수 median 23→16→**47**→29(정답을 밴드에 맞추려 모양이 자유변수가 됐다), `surface` 상 max **594** > 최상 **336**(`answerMax` 가 상·최상 둘 다 999 라 상한이 사실상 없다). ④ **시장 괴리 확정** — 상·최상이 3×3 **0%**(전부 4×4), 최상 4층 54%. 문제집 3권은 3×3 80~85%·3층 90%. 기본 난이도가 '중·상'이라 **실사용자가 보는 절반이 4×4** 다. **해법(사용자 확정) = `edu` 승격** — 랜딩 교육과정 셀렉트(교과/사고력)를 **실제 생성 축**으로 올려 `curr`=문제집 대역·`think`=현행으로 밴드를 2벌로 나눈다. **배선은 이미 전부 있다**(`generate` 가 `body.edu` 를 받고 **gsig 지문에도 포함**) — 랜딩이 URL 에 안 싣고 `gen` 이 안 읽을 뿐. 그래서 진행 중 세션·옛 공유 링크는 영향이 없다(`edu` 가 계속 `null`). **필터 정합 3건도 등재**(§5.4.1): `dim` 이 **19종 중 13종에서 조용히 무시**된다(`syncDimControl` 이 학년만 보고 유형을 안 본다 — §1.1 이 옳게 쓴 설계를 UI 가 구현하지 않았다) · 카드의 `t.levels`·`t.dim` 은 소비처 0인 죽은 필드 · S2 만 `facesMc` 게이트 구멍. **정정:** `edu` 셀렉트를 "죽은 축"으로 볼 뻔했으나 `visibleByFind:589` 가 쓰는 **정상적인 목록 필터**다 — 문제는 죽은 게 아니라 **사용자 기대와 범위가 다른** 것이다. **정상 확인(손대지 않음):** count·A-c·A-e·heightmap 단조 ✓ · surface 오목 밴딩(하·중 0%/최상 99.4%) ✓ · `answerMax` 준수 ✓ · A-a 균형 53% ✓ · 보기 4개 99.2%+ ✓ · 세션 중복 137/140 조합 0% ✓. **코드 변경 0건** — 이번은 측정·문서화까지다. |
| **v0.14.0** | **08-21** | **`dim` 을 '제시물을 정하는 파라미터'로 승격 — "지금 서비스가 이것에 충실한지 의문"(사용자)(Claude Code).** 감사 결과 **충실하지 않았다**: `dim` 은 서버 페이로드를 한 바이트도 안 바꿨다(`q.sh` 가 `gen-adapter:360` 에서 **무조건** 실림 · `body.dim` 읽는 서버 코드 0곳 · `dim` 보내는 클라 코드 0곳). 실제 역할은 "제시물의 형태"가 아니라 "클라가 어떤 뷰어를 띄울지"뿐이었고, 랜딩에서 고른 2D 는 URL 에만 있고 **서버에 전달조차 되지 않았다**. 그 어긋남이 결함 3건을 낳았다 — ① **회전 버그**(라이브 실측): 2D 의 90° 버튼이 `#iso`·`#compass`·`#rotcap` 만 갱신하고 **답란은 회전을 모른다**. `facesMc` 는 겨냥도만 돌고 보기 4개가 그대로, `heightmap` 은 `renderIso` 가 `k%2===1` 에서 바닥판을 전치하는데 답란 격자는 그대로. 진짜 기준은 "2D 인가"가 아니라 **"정답 키가 원좌표에 매여 있는가"**(heightmap·facesMc·facesDraw)인데 `noRot=force2D` 는 facesDraw 만 덮었다. ② **서버가 dim 으로 거짓말**: S2·S3 의 minmax·manip·hidden 은 `q.dim==='3d'` 인데 제시물은 **서버가 그린 정적 2D**였다(형상이 없어 3D 자체가 불가능). 원인은 `dim` 이 **스테이지 단위**인데 given 은 **유형·서브 단위**로 정해진다는 구조적 어긋남. ③ **2D 은닉 미성립**: CLAUDE.md 는 "돌려봐야 하니 형상 노출 수용"이라 적었는데 **2D 는 돌릴 일이 없어 그 전제가 안 선다**. 서버가 스스로 `'2d'` 라 판정한 facesDraw 까지 형상을 노출 중이었다. **처리(사용자 확정 = "서버가 제시물을 정한다"):** 클라가 `dim` 을 `/generate` 에 싣고, 서버 **`resolveDim(stage,type,req)`** 이 확정한다 — 최종 `q.dim` 은 **`'3d'|'2d'` 두 값뿐**(`'any'` 는 함수 밖으로 안 나간다). `GIVEN_ONLY`(hidden·minmax·manip·facesDraw)는 스테이지 무관 항상 `'2d'`(②의 거짓말 소멸) · 저학년 3D 고정이 요청보다 우선 · `'any'` 스테이지(S4·S5)만 요청을 받는다. `dim==='2d'` 면 `q.given={kind:'isoTop',iso,top,gx,gz,maxH}` 를 보내고 **형상을 빼서 2D 은닉이 성립**한다. **회전은 전면 삭제**(`ROT`·`updRot`·`renderCompass`·`ARC_CW/CCW`) — 2D 는 서버 SVG 한 장이라 돌릴 대상이 없고 3D 는 뷰어가 돌린다. 답 유일성은 유지된다(5종이 2D 가 되는 건 `capHidden`·`withTop` 이 켜진 S4·S5 뿐 → 위모양+`NOTE_HIDDEN1`). **놓치기 쉬웠던 것:** ⓐ `pr.sh` 는 `givenShape` 폴백 때문에 **항상 truthy** 라 "형상을 받았는가"의 기준이 못 된다 → `buildSession` 에서 `(gp.sh && gp.dim!=='2d')?'3d':'2d'` 로 **한 번 정규화**(옛 응답의 `'any'` 와 ②의 거짓말까지 클라가 바로잡는다). ⓑ `givenShape` 는 `edge:1` **하드코딩** → 서버 `q.edge` 없이는 edge≠1 인 부피·겉넓이가 "1cm"로 표시돼 **전원 오답**. ⓒ 2D 겨냥도는 반드시 `pr.given.iso`(서버 SVG) — 부분모양으로 다시 그리면 전 열이 높이 1인 **거짓 그림**이자 은닉 무의미. ⓓ THREE 로드 실패가 `has3D` 첫 항에 있어 **3D 유형이 조용히 '회전되는 2D'로 낙하**했다(이제 회전 없는 정적 겨냥도). ⓔ 미리보기 `pvToggleAnswer` 의 `finally{pvRender()}` 가 정답 토글마다 `pvBody.innerHTML` 을 갈아 **캔버스를 고아로 만든다** → `pvRender` **선두** dispose 가 유일한 차단점. ⓕ `.pv-back{overflow-y:auto}` 와 충돌해 `.pv3d{touch-action:none}` 없으면 태블릿에서 회전 드래그가 모달 스크롤로 먹힌다. **미리보기(사용자 확정 = "지연 로드로 진짜 3D")**: [예시] 클릭 시 three→core→viewer 순서 강제 지연 로드(모듈 스코프 프라미스 1개 = 중복·재시도 폭주 차단, 실패 시 정적 SVG 폴백), `pvFetch` 에 `dim` 을 실어 **예시와 [시작]이 같은 형태**로. `is3D` 의 거짓 안내 버그 수정(`gp.given` 미검사로 S2·S3 에 "손가락으로 돌려"가 떴다). **인접 결함 1건 동시 수정:** 모서리 길이 문구가 `run.js` 에만 있어 **`/worksheets` 독립 생성과 미리보기는 부피·겉넓이를 모서리 길이 없이** 내보내고 있었다(종이에서 답을 구할 수 없다) → `FIG.edgeNote()` 로 승격해 화면·문제지·미리보기 단일 출처화, `q.edge` 는 2D 뿐 아니라 **항상** 전송. **검증:** `resolveDim` 매트릭스 23조합 — `'any'` 유출 **0건**, 거짓 dim **5조합 교정**, 사용자 요청이 먹히는 범위 = 초6 5종. 2D 실주행(heightmap·volume·facesMc·facesDraw)에서 형상 없이 정답 높이 4까지 정상 채점, "한 모서리 = 2cm" 표시, 회전 버튼 0개. 미리보기 3D 지연 로드 후 토글·이동·열닫 반복에도 캔버스 **1개 유지**(누수 없음). **배포는 3단계** — Phase 0/1 클라(+`run.js?v=`, 없으면 옛 클라가 캐시에 남아 새 서버를 만난다) → Phase 2 서버(dual-emit: `given` 과 `sh` 를 함께, 아직 은닉 아님) → Phase 3 `q.sh` 제거(진짜 은닉, 롤백은 한 줄 복원). |
| **v0.13.1** | **08-21** | **`STAGES[].note` 폐기 — v0.13.0 이 남긴 미결 정리(Claude Code).** 랜딩이 학년 설명 줄(`#stageNote`)을 없애면서 서버 `STAGES[].note` 와 `/config` 응답의 `note` 필드가 **소비처 0**이 됐다. 데이터 필드는 지우되 문구에 담겨 있던 '이 학년은 무엇을 하는 학년인가'(유아=2~7개 눈으로 세기 · 초1~2=3~11개+위모양 고르기 · 초3~4=개수·층별 · 초5=겨냥도 첫 등장 · 초6=공간과 입체)는 **각 스테이지 줄 주석으로 옮겨 보존**했다 — 스테이지를 손볼 때 필요한 판단 근거라 값만 지우고 지식을 버리면 안 된다. `stages()` 반환과 `config/index.ts` 에서도 제거. **`/config` 응답 모양이 바뀌므로 `VERSION` cfg-0.4.0 → cfg-0.4.1** (배포본↔소스 대조가 알아볼 수 있게). 회귀: 전 스테이지·전 유형·전 등급 **2240문항 출력 완전 동일** — 생성 로직 무영향. 클라 변경 0건(이미 안 읽고 있다)이라 서버만 배포. |
| **v0.13.0** | **08-21** | **랜딩 UI 단순화 — "너무 복잡하다"(사용자)(Claude Code).** 실측 진단: 첫 화면 **클릭 가능 요소 92개**, 그중 **63개가 같은 25개 유형을 두 벌로 반복**(카테고리 아코디언 + 카드 그리드). 데스크톱 3.85화면 · **모바일 약 10화면**(폭 브레이크포인트 0개 · hover 가드 없음 · 터치 타깃 33px 로 사이트 최소). ① **아코디언 폐기 — 목록은 카드 한 벌.** 두 목록의 **필터 규칙이 달라** 개수가 어긋났고(차원 3D만 → 카드 11 / 아코디언 25) **같은 유형에 두 개의 번호**가 붙었다(아코디언 `si`=필터 인덱스, 카드 `si`=전체 인덱스). 카테고리 묶음은 **그리드 안 섹션 헤더**(클릭 0회)로 보전하고, '준비 중' 6종은 카테고리 4개에 흩어져 줄을 낭비하므로 **"곧 공개" 한 묶음**으로 맨 아래에 모았다(노출 요구는 유지). ② **필터를 [찾기]/[풀기] 두 줄로 분리.** 실측상 **목록을 바꾸는 건 학년 하나뿐**이었다 — 교육과정 '사고력수학'은 25→25(무변화), 난이도는 live 카드 0장 제거(`support()` 가 열린 유형에 항상 4등급 반환), 문항 수는 `match()` 에 아예 없었다. **눌러도 목록이 안 변하는 것이 필터처럼 생긴 것**이 복잡함의 큰 축이라 역할로 갈랐다. **찾기** = 교육과정(select)·학년(native radio)·**개념유형**(select+optgroup, 그룹마다 "○○ 전체 (n)") / **풀기** = 난이도(칩 4 복수, **기본 중·상**)·문항 수(select)·**문제 유형**(칩 2 = 3D 입체/2D 겨냥도). ⚠ **용어 주의: UI '문제 유형' = `state.dim`(제시물 3D/2D) · UI '개념유형' = `state.concept`(유형 목록).** 라벨을 그렇게 정한 이상 코드도 같이 바꿨다(옛 `state.type`→`concept`, URL `t`→`c`) — 안 바꾸면 UI 와 코드가 정반대를 말한다. ③ **'문제 유형'의 정직성** — 서버가 유형·학년별로 제시 형태를 정하므로 사용자 선택이 먹는 범위는 **초6 × 개수·부피·겉넓이·위에서 본 수·모양 고르기 5종뿐**이다. 학년이 3D 고정이면 2D 칩을 `disabled` 하고 **사유를 칩 글자 안에** 넣는다(`2D 겨냥도 (초6부터)`) — 설명 문구 줄은 필터 영역에 **0개**로 유지(사용자 확정). **🐛 폴백 `STAGE_FB` 에 `dim` 필드가 없었다** → `/config` 실패 경로에서 이 컨트롤이 거짓말을 했다. 5개 항목에 추가. ④ **카드 축소** 265px→156px: 번호 뱃지(필터로 순서가 바뀌는 그리드에서 고정 번호는 **거짓 정보**)·차원 태그(실제 dim 은 서버가 정한다)·**등급 뱃지 4개(전 카드 동일 = 정보량 0)** 제거, 글리프 46→34px. ⑤ **정리:** 이 페이지만 갖고 있던 `tokens.css` 인라인 사본 → `<link>` 전환 · 데드코드 제거(`.tabs`·`.subrow`·`.toast`+`showToast()` **호출처 0건**·`kidsOf`) · `#reset` 이 학년을 안 되돌리던 모순 수정 · **`loadURL` 전면 화이트리스트**(`?n=99` 가 `start()` 까지 갔다) · 반응형 2개 신설 · 터치 타깃 **44px**(CLAUDE.md §기술) · `@media (hover:hover)` 가드. **놓치기 쉬웠던 것:** `syncTopDOM()` 이 `[data-filter] button` 을 찾아서 select/칩으로 바꾸면 **0개 매치 → 에러 없이 아무 일도 안 일어난다**(→ `syncControls()` 재작성) · select 에 없는 값을 `value` 로 넣으면 **조용히 빈 칸**(→ 검증이 선행) · native radio 는 화살표 키로 `click` 없이 바뀌므로 위임을 `change` 로 · 난이도 칩을 **전부 끄면** `start()` 의 `lvls.length?lvls:sup` 때문에 조용히 전 등급으로 시작(→ 최소 1개 강제). **결과:** 클릭 92→**59** · 데스크톱 3.85→**2.2화면** · 모바일 약 10→**약 4.1화면** · 터치 33→44px. **예시 미리보기는 한 줄도 건드리지 않았다.** ℹ `#stageNote` 를 없애면서 서버 `STAGES[].note` 와 `/config` 의 `note` 필드가 **소비처를 잃었다**(→ v0.13.1 에서 정리 완료). 서버 변경 0건. |
| **v0.12.2** | **08-21** | **그리기 유형 결함 3건 — 사용자가 라이브에서 직접 풀어 보고 신고(Claude Code).** ① **해설 그림이 답안 격자와 크기가 달랐다.** 아이는 `격자폭 × 등급 maxH` 를 칠하는데 해설은 `renderThreeViews(silsOf())` 를 써서 **실제 최고 높이**로 그려졌다(`frontSil.rows = Math.max(...막대)`). 실제 높이 1인 문항에서 답안 4×4 · 해설 **4×1 띠** → 나란히 비교가 불가능. 이미 올바른 렌더러(`renderDrawGrids`, `drawDims` 기반)가 미리보기·문제지 정답지에 쓰이고 있었으므로 퀴즈 화면 해설 3곳을 거기에 합류시켰다(화면 24px / 문제지 18px = 각 답안 격자와 같은 값). ⚠ `frontSil`/`sideSil` 의 rows 는 건드리지 않았다 — 같은 함수가 G-a 삼면도 제시물·G-b·facesMc 보기 4개에도 쓰인다. **함께 고쳐진 것:** facesDraw 해설이 `views` 를 안 넘겨 **한 면만 물어본 등급에서도 삼면을 다 그리고 있었다**. CSS 는 `.sol-draw .sol-pic span` 이 자손 선택자라 새 격자의 방향 라벨까지 먹어서 자식 선택자로 좁혔다. ② **겨냥도만으로 위에서 본 모양이 정해지지 않았다.** 숨은 열은 `flattenInBand` 가 없앴지만 **비어 보이는 칸**은 안 봤다 — 빈 칸의 앞대각이 2층 이상이면 그 자리 1개짜리 나무는 그림에 전혀 안 나타난다(가림 규칙 `(x+1,y+1,z+1)`). 실측 G-c·H-a·H-b 의 **약 절반**이 이런 칸을 갖고 있었다. **사용자 결정: 단서를 다는 대신 문항 자체에서 배제** → `cubenest-hidden` 에 `ambiguousEmpties`/`isDeterminate` 신설(격자 크기 인자 불필요 — 2층 이상 칸에서 뒤로 한 칸 물러나 보므로 항상 격자 안), 기존 재시도 루프 2곳(G-c·H-a/H-b)의 수용 조건에 AND. **발문은 안 바꿨다.** 실측 1회 수용률 42~67% → 60회 루프 실패확률 10⁻⁹ 미만, **14,000문항 애매빈칸 0 · 숨은 열 0**, 개수 밴드 이탈은 종전과 같은 수준(≤3.8%). 대안이던 '앞대각을 1층으로 내리는 수리'는 밴드를 15~30% 깨뜨려 기각. **🐛 G-c 는 조건만 걸었을 때 1~4% 가 그대로 통과했다** — H-a 에서 겪은 것과 같은 원인(`resolveCfg` 가 발자국·개수를 하나로 고정해 60번 재시도가 같은 종류만 낳는다). H-a 에 인라인돼 있던 완화를 `widenCfg()` 로 승격해 두 루프가 함께 쓴다. **손 안 댄 것:** `manip/H-c` 는 `genCubeGap` 의 비증가 높이 구성으로 **이미 100% 결정적**(실측) · ISO 6종은 위모양이 발자국을 알려 주므로 해당 없음 · `hidden/A-a` 는 가림 유무가 곧 정답이라 제외. ③ **facesDraw 문제 화면을 2D 고정으로.** 위모양+단서가 이미 답을 유일하게 만드는데 3D 를 앞·옆으로 돌리면 **답 실루엣이 그대로 읽혀** 문항이 무의미해진다. 서버 `dim` 을 확장해(`questionFor` 에서 facesDraw 만 `"2d"`) 판단의 단일 출처를 유지했다 — `"2d"` 는 그동안 미사용 값이었고 이번에 처음 의미를 갖는다. 클라는 `force2D`(→ URL `?dim=3d` 로도 못 뚫음)와 `noRot` 만 읽는다. **회전 버튼은 없앴다** — 버튼이 `#iso` 만 돌리고 답란은 안 돌리는데 facesDraw 는 답이 방향에 매여 있어 돌려 놓고 그리면 기준이 어긋난다(개수 유형은 방향 무관이라 지금껏 문제가 안 됐다). 핸들러에 null 가드도 추가(`.onclick` 직접 대입이라 요소가 없으면 문항이 통째로 빈 카드가 된다). 의견 폼의 `viewLabel` 중복 식을 없애고 렌더가 정한 값을 재사용(2D 문항에도 "3D 문제"로 찍히던 것). 랜딩 카드 `dim:'both'→'2d'`, 미리보기의 "돌려 보세요" 문구 제거. **해설의 3D 뷰어는 유지**(`VIEWER && window.THREE` 만 보므로 영향 없음, 실측 확인). **회귀:** 전 스테이지·전 유형 대조에서 **바뀐 것은 G-c·H-a·H-b 뿐**이고 나머지 16종(count·volume·surface·heightmap·facesMc·facesDraw·A-a~f·G-a·G-b·H-c·H-d)은 완전히 동일. ⚠ **재현성:** G-c·H-a/H-b 는 같은 seed 로 다른 문항이 나온다(§6.4 예외 — v0.12.0 중복 회피와 같은 성격). ⚠ facesDraw 에서는 `quiz_rotate`·`quiz_view_reset` 둘 다 발화하지 않는다(2D 고정 유형은 `dim` 으로 구분된다). |
| **v0.12.1** | **08-21** | **H-a·H-b 초6 개방 + 그 과정에서 드러난 생성기 결함 2건(Claude Code).** 기준은 **"그리기가 가능한 연령대"**(사용자 결정) = `facesDraw` 가 열린 스테이지 = 현재 S4. `GATE.S4.manip` 에 `H-a`·`H-b` 추가(서버 + 랜딩 폴백 `GATE_FB` **동시** — 한쪽만 고치면 `/config` 실패 시 카드가 사라진다). **🐛 등급 정책이 두 파일에서 반대말을 하고 있었다** — `support()` 는 "열려 있으면 4등급 전부"인데 `gen.js` 는 `o.level === '상'` 일 때만 H-a/H-b 를 뽑았다(v0.10.2 의 '최상 제외' 잔재). 카드는 4등급을 파는데 랜덤 추첨은 상에서만 나오는 어긋남이라 **제한을 폐기**하고 게이트로 일원화했다(카드 `levels:['상']` 도 교정). **🐛 H-a 하·중 3% 가 H-c 로 대체되고 있었다** — `resolveCfg` 가 발자국 `f` 와 개수 `n` 을 **하나로 고정**하는데 `n = f×maxH`(모든 열이 높이 상한)면 더 쌓을 자리가 없고, 같은 cfg 로 60번을 다시 뽑아도 결과가 같다(실패 9/9 전부 이 포화). 첫 실패에서 cfg 를 개수 밴드 전체로 넓히도록 고쳐 **400/400 대체 0**. **🐛 그 H-c 대체 자체가 망가져 있었다** — 정상 H-c 는 `genCubeGap` 으로 정답값을 `needBand` 에 맞춰 역산하는데 폴백은 현재 모양에 `completeCube` 를 돌려 S4 상에서 정답이 46~52(밴드 [10,25])가 되고, 이미 정육면체면 `need=0` 인 출제 불가 문항이 나왔다 → `buildHc()` 로 두 경로를 통합(실측 need 이탈 0/1600 · need=0 0). **🐛 미리보기·문제지에서 "빨강으로 표시한 줄" 안내가 사라졌다** — 이 문구가 `run.js` 에만 있어 `figures.renderGiven` 경로에는 없었다 → `figures.givenCaption()` 으로 승격하고 run 이 위임(`isoMark` 는 '어느 줄인가'가 빨강뿐이라 이 한 줄이 없으면 문제가 성립하지 않는다). run 헤더 배지에 `isoMark` 라벨 추가. **회귀 검증:** 기존 5스테이지 전 유형·전 등급 **3168문항이 리팩터 전후 완전 동일**(결정성·하위호환). |
| **v0.12.0** | **08-21** | **저학년 확장(유아·초1~2) · 예시 미리보기 · 세션 내 중복 회피(Claude Code).** ① **S0(유아 5~7)·S1(초1~2) 개방, S5(중1~2) 비노출**(설계·밴드는 코드에 보존). 근거 = 시중 유아 사고력 '도형' 교재 실측(개수 2~8·2층·바닥 2×2~3×3). 여는 유형은 `count`·`facesMc` **2종뿐** — 개수만 줄인다고 저학년용이 되지 않는다. 대상 만 8~13세 → **만 5~11세**(난이도 조정이 아니라 유아 시장 진입). ② **`facesMc` 고유축 `dirs` 신설** — S0=위만, S1=위·앞. 앞·옆은 높이를 추론해야 하는 삼면도 개념이라 이르고, 위(발자국 찾기)가 유아 교재의 표준 문항이다. ⚠ S1 '하'를 XS(2×2)에 두면 '앞' 보기가 2~3개밖에 안 나온다(2칸 막대 실루엣이 8가지뿐, 실측 2.9%) → S(3×3). ③ **예시 미리보기(§1.1)** — 랜딩 카드·목록의 [예시 보기]가 **실제 `/generate` 문항 3개**를 모달로 보여준다. 정답은 `/grade` 로만 받는다(§3.12 은닉 유지). 그림은 `figures.renderQuestion` — 퀴즈 화면·문제지와 같은 렌더러(`answerText` 도 공용 모듈로 승격, worksheets·run 이 위임). ④ **학년 목록의 단일 출처를 `/config` 로** — 랜딩이 칩을 서버 응답으로 그리고 닫힌 학년 링크는 기본값으로 되돌린다(폴백표는 서버와 자동 대조로 일치 확인). ⑤ **세션 내 중복 회피(gen v0.4.0→v0.5.0)** — 8문항 세션 중복률 S0 하 100%→7%, S1 하 90%→0%, **S2 `count` 하 61%→0% · S2 `heightmap` 하 78%→0%(이미 배포돼 있던 결함)**. ⚠ 중복 판정은 모양을 **점유 범위로 정규화**해야 한다(원 JSON 키로는 100%→100%). ⚠ 중복이 있던 옛 seed URL 은 다른 문항을 낸다(마스터 §6.4 재현 규약 1회 예외). **🐛 랜딩에 `track()` 정의가 없어** 미리보기가 ReferenceError 로 통째로 죽었다(모달만 뜨고 본문 영구 공백) → `quiz/run` 과 같은 가드형 `track()` 추가. **🐛 카테고리 아코디언이 학년 게이트를 무시**해(카드 격자만 적용) 유아에게 '안 보이는 나무'가 보이고 눌렸다 → 같은 `gateFor` 적용(유아 노출 19종→2종). **🐛 미리보기 연타 시** 늦게 온 응답이 이미 바뀐 유형 위에 그려졌다 → 요청 토큰(`PVREQ`). **🐛 그리기 정답이 안 물어본 '위'까지** 그렸다 → `views` 만. **⚠ 배포 순서 사고:** 서버만 먼저 배포해 라이브에서 옛 클라가 **없어진 중1~2를 계속 팔았다**(풀 수 있는 유형 0개, 목록으로 시작하면 `normStage()` 가 S4 로 떨어뜨려 초6 문제가 조용히 나옴) → CLAUDE.md 에 **'클라 먼저, 서버 나중'** 규약 신설. **🔴 미결:** S5 를 닫으면서 **H-a·H-b 가 도달 불능**이 됐다(S5 에만 열려 있던 서브) — 위 표 참조. |
| **v0.11.5** | **08-19** | **인쇄 실촬영 4건 + 완주 흐름 자동검증 2건(Claude Code).** 헤드리스 Chrome 으로 문제지를 인쇄 미디어에서 실제 촬영. **네 건 다 데이터 검증은 통과하고 있었다.** ① 삼면도가 10px 로 찌그러짐 — `renderSil` SVG 는 `viewBox` 만 있고 고유 폭이 없어 flex 안에서 0 으로 줄어든다 → 폭 명시. ② 오른쪽 절반이 비어 6문항이 A4 두 장 → `@media print` 2단(시중 문제집이 전부 2단인 이유). ③ `facesDraw` 발문("위·앞·옆")과 답란(앞·옆 2칸)이 어긋나 아이가 없는 칸을 찾게 됨 → 발문을 `views` 에서 생성. ④ **확정 결정 위반** — `withTop` 을 스테이지로 게이트하지 않아 S2·S3 에도 위모양이 실렸고 그 결과 **S3 의 `facesMc` 가 위·앞·옆 전부를 못 냈다**(§7.9-2) → `capHidden` 과 같은 조건으로 묶음. ⓘ 검증 스크립트는 라벨만 잘못 달고 이 상태를 정상으로 출력하고 있었다 — 화면을 안 봤으면 못 잡는다. 이어서 **quiz 완주 흐름 자동검증**(제출→채점→결과→문제지): 5/5 정답으로 서버 채점이 클라 형상과 일치. **🐛 quiz 위임 문제지에 위모양이 안 실렸고**(`figureOf` 가 겨냥도만 그림) payload 에 `top` 이 있어 문제지는 "지면에서 풀린다"고 판단해 QR 도 안 붙였다 — **위모양도 QR 도 없는 최악의 조합**. **🐛 겨냥도가 아주 작게 인쇄** — `.isofig` 로 감싸야 크기 규칙이 먹는데 위임 경로만 빠져 있었다. |
| **v0.11.4** | **08-19** | **문제 품질을 시중 문제집 수준으로 · 겨냥도 단독 출제 폐지 · 문제지 QR(Claude Code).** 시중 문제집 3종 64면 실측(바닥 3×3 이 80~85%·최고 3층 90%·세는 유형 7~13개) — 우리 S4 최상(26~36)은 문제집 '심화' 대역(최대 23)마저 넘고 있었다 → 개수 밴드를 한 스테이지씩 내리고(S4 = 6~9/9~13/12~18/16~24) 격자는 S(3×3) 주력으로 되돌림. ⭐ **겨냥도 단독 출제 폐지** — 문제집 3권이 예외 없이 지키는 규칙(겨냥도만 주고 개수를 묻지 않는다, 반드시 위모양을 함께 주고 숨은 자리는 1개로 규약)을 채택. `capHiddenToOne()` 신설, **겨냥도 6종에만** 적용. ⚠ `hidden`·`minmax`·`manip` 에는 적용 금지 — A-f 정답이 숨은 열의 자유도 곱이라 높이를 1로 고정하면 정답이 항상 1이 된다. 이것이 "3D 회전 6종은 종이에서 답이 결정되지 않는다"의 교과 해법이며, 가림을 남기므로 '안 보이는 나무' 개념도 산다. 위모양이 곧 top 의 정답이라 S4·S5 는 묻는 방향에서 top 을 뺀다(`facesMc` dir∈{앞,옆} / `facesDraw` 상·최상은 앞·옆 2면) = 문제집 표준 조합. 발문을 [상황 선언]+[요구] 2문장으로 고정하고 단서는 문말 별행 괄호로. |
| **v0.11.3** | **08-19** | **난이도 개편 1·2단계 — 연령 스테이지 축 + 유형별 고유축(Claude Code).** 축 순서를 뒤집었다: `등급 → (격자,밀도) → 개수(결과)` → **`스테이지 → 개수 밴드(1차) → 격자·높이 → 등급`**. 밀도 라벨(full/mid/sparse) 폐지 — '꽉참=규칙적=쉬움' 전제가 구현에서 **0.9%만** 성립했다(full 이 직육면체가 아니라 85~100% 채움이었다). `makeShape()` 신설(재생성 루프까지 전부 이걸 쓴다 — `genShape` 를 직접 부르면 그 자리에서 스테이지 규칙이 샌다). 유형 고유축: `heightmap` 1차 축을 개수→**칸 수**로 교체(고유축 = 굴곡, 실측 0.30→0.74→1.05→1.32 단조) · **A-f 가짓수 밴드**(정답이 {2,3,4,6} 네 값뿐이라 찍어서 25% 맞던 것) · **H-d 직육면체 일반화** · H-c 는 답이 `m³−개수` 라 개수 밴드로 제어 불가 → 목표 답에서 거꾸로 계단 모양 생성 · `facesDraw` 출제 형식(모양 1개면 세 면 / 여러 개면 각 한 면). **`/config` 신규** — 열린 스테이지·유형·서브·지원 등급의 단일 출처(랜딩이 `levels` 를 하드코딩해 'count 상' 같은 유령 등급을 팔던 것을 끝냄). `stage` 를 generate·grade·worksheet 에 흘리고 **gsig 지문에 포함**(어긋나면 403 으로 즉시 드러남, 단 있을 때만 덧붙여 기존 세션 보존). 실측: `count` 하 median 25→10 · `volume` 중 최대 1701→144 · 등급 역전 3유형→0. 상세 = `cubenest_난이도_재설계_260818_v0_1.md`. |
...
| 입력UI | 위·앞·옆 그리기 (`facesDraw`) | 격자 칠하기 / 칸별 실루엣 | ✅ (최상 제외는 v0.11.3에서 폐기) |
| 사고력 | **최소·최대 3종** (`minmax`/`G-a`~`G-c`) | 숫자 / `reverseCounts`·`minmaxFromTopAndSil`·`countAtLeast`(4.4·§3.6) | ✅ |
| 사고력 | **안 보이는 나무 6종** (`hidden`/`A-a`~`A-f`) | bool·markCells·num / §3.7 | ✅ |
| 사고력엔진 | **조작 4종** (`manip`/`H-a`~`H-d`) | 숫자·drawSil / §3.6.2 | ✅ (H-a·H-b 는 v0.12.1 에서 초6 개방) |
| 사고력엔진 | J(폴리큐브) | 회전 동일성 | ⏳ P2 |
| 공통 | **연습장**(전 유형·2레이어) | 자유 필기 canvas(벡터) | ✅ |
| 공통 | **예시 미리보기**(랜딩) | 실제 `/generate` 3문항 · 정답은 `/grade` | ✅ (v0.12.0 · §1.1) |

> 랜딩 카드 25종 중 **구현 19 / 곧 공개 6**(그림자 추론·무엇을 바꿨나·모양 만들기·조각 맞추기·같은 모양 찾기·가짓수 세기).
>
> ⚠ **구현 19종이 곧 도달 가능 19종은 아니다.** 여는 유형은 스테이지가 정한다(`GATE[stage]`, 단일 출처 = 서버 `/config`). 실측:
> | 학년 | 도달 가능 |
> |---|---|
> | 유아 · 초1~2 | **2종** (`count`·`facesMc`) |
> | 초3~4 | **5종** (`count`·`heightmap`·`hidden` A-c/A-e·`minmax` G-c) |
> | 초5 | **7종** (+ `facesMc`·`manip` H-c) |
> | 초6 | **19종** (전 유형) |
>
> ✅ **H-a·H-b 는 v0.12.1 에서 초6에 열었다**(사용자 결정 260821: "그리기가 가능한 연령대 모두"). 기준 = **`facesDraw` 가 열린 스테이지** — 답이 삼면도라 그리기가 성립하는 학년이 곧 이 둘이 성립하는 학년이다. 지금은 S4 뿐이고, 나중에 다른 학년에 `facesDraw` 를 열면 함께 연다. (worksheets **독립 생성**은 `PAPER_UNSAFE` 로 계속 제외 — 표시 줄이 빨강 색상뿐이라 흑백 인쇄에서 지시 대상이 사라진다. quiz 위임 문제지는 QR 로 이어지므로 무관.)

---

## 1. 두 화면

### 1.1 설정 — `quiz/index.html`  (v0.13.0 단순화)
- **헤더 = 공용 헤더**(`nav.css` + 인라인 마크업, 정본 `account/index.html`). 게이트는 걸지 않는다(퀴즈는 무로그인).
- **토큰은 `tokens.css` 링크**를 쓴다. v0.13.0 전까지 이 페이지만 인라인 사본을 갖고 있었다.
  ⚠ `.glyph`/`.brand` 의 나무색·방향색 변수는 tokens.css 에 없으므로 인라인으로 남긴다.
- **필터는 두 줄이고 역할이 다르다. 이 계약을 깨지 말 것.**
  - **[찾기]** — 바꾸면 아래 카드 목록이 바뀐다: 교육과정(select) · 학년(native radio, 서버 `/config` 로 그린다) · **개념유형**(select + optgroup, 그룹마다 "○○ 전체 (n)")
  - **[풀기]** — 목록을 바꾸지 않고 '시작'할 때만 적용된다: 난이도(칩 4 복수, 기본 **중·상**, 최소 1개) · 문항 수(select) · **문제 유형**(칩 2)
  - ⚠ `match()` 에 난이도·문제유형 조건을 넣으면 이 계약이 즉시 깨진다. 특히 **soon 카드에 `t.levels` 검사를 걸면** 난이도만 바꿔도 '준비 중'이 사라진다.
- ⚠ **용어**: UI **'문제 유형'** = `state.dim`(제시물이 3D 입체인지 2D 겨냥도인지) · UI **'개념유형'** = `state.concept`(유형 목록). 이름이 서로 얽히기 쉬우니 코드를 고칠 때 반드시 확인할 것.
- **'문제 유형'이 먹는 범위는 좁다** — 서버 `q.dim` 이 URL 보다 우선한다. 유아~초5는 3D 고정, `facesDraw` 는 2D 고정, minmax·hidden·manip 은 항상 2D → 실제 선택이 유효한 것은 **초6의 count·volume·surface·heightmap·facesMc 5종**. 3D 고정 학년에서는 2D 칩을 `disabled` 하고 사유를 칩 글자 안에 넣는다. **필터 영역에 설명 문구 줄은 두지 않는다.**
  - ⚠ **그런데 화면이 그 좁은 범위를 반영하지 않는다(260831 확인, 미수정).** 비활성 처리를 하는 `syncDimControl`(`:551-560`)이 **학년만** 보고 유형을 안 본다. 그래서 초6에서 `hidden`·`minmax`·`manip`·`facesDraw` **13종의 3D 선택지가 눌리고**, 서버는 그것을 조용히 무시한다. 위 문장이 옳은 설계인데 **UI 가 그 설계를 구현하지 않은 상태**다 — `GIVEN_ONLY` 4유형까지 `disabled` 조건에 넣어야 한다. 상세 = §5.4.1.
- **목록은 카드 한 벌**이고 카테고리 섹션 헤더로 묶는다. 카드 요소 = 글리프·이름·한 줄 설명·(사고력 전용 태그)·[예시][시작]. 번호 뱃지·차원 태그·등급 뱃지는 폐지(각각 거짓 정보 / 서버 소관 / 전 카드 동일).
- **'준비 중' 6종은 "곧 공개" 한 묶음**으로 맨 아래(노출은 유지).
- 시작하기 → `./run/?type&sub&lv&n&stage&dim`. 필터 상태는 URL(`edu`·`c`·`lv`·`n`·`stage`·`dim`)로 공유되고 **전부 화이트리스트 검증**한다.
- **터치 타깃 44px** · 반응형 브레이크포인트 640/400px · hover 는 `@media (hover:hover)` 안에만.
...
- **`cubenest-figures`(제시물 도형 SVG, v0.2.0):** `renderSil`·`renderThreeViews`·`renderTopNums`·`renderLayers`·`renderDrawGrids`·`shapeFromGiven`·**`renderGiven(given)`** — 제시물 종류를 알아서 그린다(worksheets 가 19종 분기를 갖지 않는 이유). v0.2.0 에서 **`renderQuestion(q)`**(제시물+`facesMc` 보기 한 덩어리)·**`answerText(key,unit)`**(정답 한 줄) 승격 → 랜딩 미리보기·run·worksheets 세 곳이 같은 것을 쓴다. ⚠ 겨냥도는 `cubenest-iso` 소관이지만 `renderQuestion` 만 예외로 `CubeNest.iso` 를 **있으면** 쓴다.
- **`cubenest-viewer`(뷰어/펼쳐보기, v0.2.3):** `createViewer(...)` + 펼쳐보기 + `setHighlight` + `dirLabelPositions`.
- **`cubenest-qr`(v0.2.0):** 문제지 QR(지면에서 3D 가 필요한 문항을 화면으로 잇는다). worksheets 소관.
- **`auth`(v0.1.0)·`mydata`(v0.1.0)·`consent`:** 인증 단일 진실 · 로컬 우선 데이터 계층 · GA4 동의. ⚠ **랜딩(`quiz/index.html`)은 `consent.js` 를 싣지 않는다** — 그래서 `track()` 을 페이지 안에 가드형으로 따로 정의한다(§1.1).
...
- **`cubenest-gen`(생성기, v0.5.0):** `rngFrom` · `genShape` · **`reshape`** · **`makeShape`**(스테이지 규칙 준수 — `genShape` 를 직접 부르면 규칙이 샌다) · `isConcave` · `genProblem`/`genSession`(**세션 내 중복 회피** 포함).
- **`cubenest-gen-config`(cfg-0.4.0):** `STAGES`(연령 스테이지·`open`·`note`) · `BANDS`(스테이지×등급 개수 밴드) · `GATE`(유형·서브 게이트) · `TYPE_AXES`(유형 고유축 — `edge`·`answerMax`·`concave`·`foot`·`kinds`·`dirs`·`boxTotal`…) · `stages`/`typesFor`/`subsFor`/`support`/`paperSafe` · `resolveCfg` · `normStage`.
- **`cubenest-hidden`:** `visibleTopFootprint` · `hiddenColumns` · `hasHidden` · **`flattenHidden`** · **`capHiddenToOne`**(가림 유지·높이만 1 — v0.11.4) · `enumerateByVisible` · `layersToShape`.
- **`cubenest-minmax`:** `countAtLeast`(G-c) · `minmaxFromTopAndSil`(G-b) · `groupByAxis`. **`cubenest-manip`(v0.2.0):** `completeCube`(H-c) · `paintedBoxCount`/`paintedCubeAll`·`solidBoxHmap`(H-d 직육면체 일반화).
  - ⚠ 이 둘은 **서버 단일본**이다. 클라 사본은 어떤 페이지도 로드하지 않는 죽은 코드였고 서버본만 H-d 일반화가 들어가 조용히 갈라져 있어 삭제했다(마스터 §5.2 규칙 4).
- **`cubenest-core`(복본)·`cubenest-iso`(복본)** · `gen-adapter`(`buildProbs`·`answerKeyFor`·`checkAnswer`·`questionFor`·`explainFor`) · `gsig`·`rate`·`cors`.
...
- **로딩 순서(실제):** `run` = `THREE → core → iso → figures → api-client → viewer → (본문) → supabase-js → auth → mydata → consent → run.js` / **랜딩** = `supabase-js → auth → iso → figures → api-client`(미리보기용, Three.js 없음). 캐시 무효화 `?v=`(**core 0.3.0 · iso 0.1.0 · figures 0.2.0 · viewer 0.2.3 · qr 0.2.0**).
  - ⚠ `?v=` 는 **세 페이지가 같은 값**이어야 한다. `answerText` 가 `figures` 로 옮겨 간 뒤 한 페이지만 옛 캐시를 받으면 **정답지가 통째로 "—"** 로 죽는다(v0.12.0 에서 0.2.0 으로 통일).
...
#### 5.4.0 ⛔ 출제 품질 결함 (2026-08-31 전수 실측 · 정본 = `난이도_재설계 §11`)

> 19종 × 4등급 × 5스테이지를 500문항씩 다시 쟀다. 아래는 **전부 실측**이고, 우선순위·수정 위치는 §11.8 에 있다.
> ⚠ **이 절의 1~3번은 "아이가 정답을 낼 수 없거나, 안 풀고 맞히는" 부류다.** 다른 어떤 개선보다 앞선다.

| # | 결함 | 실측 | 위치 |
|---|---|---|---|
| **1** | **`sub` 가 서버에서 검증되지 않는다.** `dim`·`stage` 는 화이트리스트로 받는데 `sub` 만 `body.sub` 를 그대로 넘긴다. `?type=hidden&sub=A-b&stage=S2` → S2 는 `flatten` 이라 숨은 열이 없어 **100% 성립 불가**. 랜딩은 막지만 공유 링크·문제지 QR·직접 진입이 뚫려 있다 | — | `generate/index.ts:37` ← `gen-config.js:224 subsFor` 가 이미 답을 안다 |
| **2** | **A-b(숨은 칸 찾기) 정답 0칸** — 찍을 칸이 없어 제출 자체가 안 된다(`run.js:422`). URL 조작 없이 **정상 경로**에서 난다 | 하 **14%** · 중 3.6% · 상 4.4% · 최상 1.6% | `gen.js:430` — `widenCfg` 없이 고정 cfg 를 60회 반복 |
| **3** | **A-d·G-a 정답 0** — `max===min` 인데 `which='diff'` 로 "차"를 묻는다. `which` 결정이 재시도 루프와 **독립** | 하 **10.8%** | `gen.js:399-400`(G-a) · `:432-434`(A-d) |
| **4** | **A-f 정답이 등급 안에서 고정** — 안 풀고 등급만 알면 맞는다 | 하 **500/500 이 "2가지"** · 중 98.4% 가 "4가지" | `gen-config.js TYPE_AXES.hidden` |
| **5** | **H-d 최상 문항 공간이 4개** — 상자 4×4×4 하나, 정답 {8,24} 두 값. **10문항 세션의 100% 에서 중복** | 최빈 51.4% | `gen.js:501-503` (`cap=maxH` 라 5×4×4 를 못 쓴다) |
| **6** | **H-c 제시물 개수 비단조·폭주** — 답을 작게 하려면 모양을 크게 해야 한다 | median 하 23 → 중 16 → **상 47** → 최상 29 | `gen.js:171-190 genCubeGap` |
| **7** | **surface·volume 상이 최상을 넘는다** — `answerMax` 가 둘 다 999 라 상한이 사실상 없다 | surface 상 max **594** > 최상 **336** | `gen-config.js TYPE_AXES` |
| **8** | **규격이 시장 대역 밖** — 상·최상이 3×3 **0%**(전부 4×4), 최상 4층 54%. 문제집은 3×3 80~85% · 3층 90%. 기본 난이도가 '중·상'이라 **실사용자가 보는 절반이 4×4** | — | `gen-config.js:74-89 BANDS` |
| **9** | **`facesMc` 오답 강도가 등급을 모른다** — `makeSilOpts` 에 cfg 가 안 넘어간다. S0 는 L2~L4 오답 거리가 **정확히 1.00 고정** | S0 1.00 / S4 3.04 (스테이지 부수효과) | `gen-adapter.ts:46` |
| **10** | `facesMc` 보기 3개 발생 | 0.2~0.8% | `gen-adapter.ts:48` (80회 소진) |
| **11** | `facesDraw` 그룹 인덱스가 **재시도 소금이 붙은 seed** 를 쓴다 → "모양 2~3개 × 같은 한 면"(§4.8) 이 깨진다 | — | `gen.js:349` |

**해법의 축 = `edu` 승격(§11.7 확정).** 랜딩의 교육과정 셀렉트(교과/사고력)를 **실제 생성 축**으로 올려
`curr` = 문제집 대역(3×3 주력), `think` = 현행 유지로 밴드를 2벌로 나눈다. **배선은 이미 전부 있고**
(`generate` 가 `body.edu` 를 받고 gsig 지문에도 포함) 랜딩이 URL 에 안 싣고 `gen` 이 안 읽을 뿐이다.

#### 5.4.1 ⚠ 필터가 약속을 안 지키는 곳 (2026-08-31)

- **`dim`(문제 유형 3D/2D)이 19종 중 13종에서 조용히 무시된다.** `syncDimControl`(`quiz/index.html:551-560`)은
  **학년만** 보는데, 서버 `resolveDim`(`gen-adapter.ts:298,305`)은 `GIVEN_ONLY = {hidden, minmax, manip, facesDraw}`
  를 **유형만 보고 무조건 `2d`** 로 확정한다. S4 에서 `hidden-Aa` 를 "3D 입체"로 골라도 2D 가 온다.
  정답의 단일 출처가 서버라 **오채점은 없지만 필터 약속이 거짓**이다.
  → 고칠 방향: `syncDimControl` 이 **카드 유형까지** 보고 `GIVEN_ONLY` 4유형의 3D 선택지를 `disabled` 한다
  (학년 기반 비활성이 이미 쓰는 패턴). §1.1 의 "'문제 유형'이 먹는 범위는 좁다"는 서술을 이 조건으로 좁힐 것.
- **카드의 `t.levels`·`t.dim` 은 죽은 필드다.** `levelsOf()`(`:601`)가 `gateFor()` 를 우선하고 `t.dim` 은 소비처가 0이다.
  카드 선언과 실제 동작이 **서로 다른 진실**을 갖는다 → 삭제.
- **문항 수 클램프가 3층 각각 다르다** — 랜딩 10/20/30 · `run.js:362` `[5,30]` · `generate:17` `[1,30]`.
- **S2 만 `facesMc` 가 닫혀 있다**(`gen-config.js:103`). S0·S1(방향 제한)·S3(전 방향)은 열려 있는데 S2 만 건너뛴다.
  주석에 사유가 없어 **오탈락으로 보인다** — 열면 `/config` 가 정본이므로 **서버 먼저 배포**.
- ✅ **`edu` 셀렉트는 죽은 축이 아니다** — `visibleByFind:589` 가 쓰는 정상적인 [찾기] 목록 필터다.
  다만 사용자가 **생성에도 영향을 줄 것으로 기대**하므로 §5.4.0 의 해법으로 승격한다.

#### 5.4.2 기존 미결
- 🟢 **겨냥도 결정성 규칙(v0.12.2).** 겨냥도만으로 모양이 유일하려면 ① 숨은 열 없음 ② **비어 보이는 칸의 앞대각이 2층 미만** 두 가지가 필요하다. `cubenest-hidden.isDeterminate` 가 단일 출처이고, 2D 로 겨냥도만 주는 유형(G-c·H-a·H-b)이 생성 루프에서 이것을 통과한 모양만 쓴다. **새 유형이 겨냥도를 2D 로 단독 제시하면 이 조건을 반드시 걸 것.**
- 🟡 **H-a·H-b 의 답안 부담.** 세 면을 모두 그려야 해서 답안 칸이 하·중 27 · 상·최상 48 이다(같은 학년 `facesDraw` 는 9 / 32). `facesDraw` 가 쓰는 출제 형식 축(`DRAW_FORM` — 모양 여러 개 × 각 한 면)을 H-a/H-b 에도 적용하면 하·중을 9~16 으로 낮출 수 있다. **난이도 설계 변경이라 보류** — 필요하면 §4.8 과 함께 다룬다.
- 🟡 **랜딩 유형 수 표기.** 유아에서 "8개 유형"으로 보이지만 실제로 풀 수 있는 것은 2개이고 나머지 6개는 '준비 중'이다. 전 학년에서 그랬던 동작이나 저학년에서 비율이 뒤집혀 읽기가 나쁘다. **사용자 결정(260821): 현행 유지** — '준비 중' 표기를 그대로 둔다.
- **worksheets 구현**(`fromQuiz` PDF·문제 그림 렌더 공유·정답지·PDF 엔진) — 스켈레톤 `worksheets_기능개발명세서 v0.2.0`.
- 연습장 **유형별 on/off**(`NO_SCRATCH`).
