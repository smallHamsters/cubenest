...
> **버전:** v0.12.0 · **상태:** 초안 — 본문 = 현행 **19유형**(계산 3 + 위앞옆 3 + G군 3 + A군 6 + H군 4) · **최종 수정일:** 2026-08-21
...
| 사고력엔진 | **조작 4종** (`manip`/`H-a`~`H-d`) | 숫자·drawSil / §3.6.2 | ⚠ **H-c·H-d만 도달 가능** — 아래 참조 |
...
> | 초6 | **17종** (전 유형 − H-a·H-b) |
>
> 🔴 **H-a(더 쌓은 후 그리기)·H-b(빼낸 후 그리기)는 지금 어느 학년에서도 나오지 않는다.** 구현·검증은 끝났는데(v0.10.2) `GATE` 상 S5(중1~2)에만 열려 있었고, v0.12.0 에서 S5 를 닫으면서 **함께 도달 불능이 됐다**. 의도한 결과가 아니다 — 초6에 열지, 계속 닫아 둘지 결정이 필요하다. (worksheets 독립 생성은 `PAPER_UNSAFE` 로 원래 제외 대상이라 영향 없음: 표시 줄이 빨강 색상뿐이라 흑백 인쇄에서 지시 대상이 사라진다.)
...
### 5.4 미결
- 🔴 **H-a·H-b 를 어디에 열 것인가(v0.12.0 발생).** 구현·검증은 끝났는데 `GATE` 상 S5(중1~2)에만 열려 있었고 S5 를 닫으면서 **어느 학년에서도 안 나온다**. 선택지 = ① 초6(S4)에 연다(가장 자연스럽다 — '상' 등급 사고력 유형) · ② 계속 닫아 둔다(그러면 코드가 죽은 채로 남는다) · ③ 중1~2 를 다시 연다. ※ worksheets 독립 생성은 원래 `PAPER_UNSAFE` 제외 대상이라 영향 없다.
