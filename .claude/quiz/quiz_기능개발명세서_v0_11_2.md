...
> **버전:** v0.11.2 · **상태:** 초안 — 본문 = 현행 13유형(8유형 + 안 보이는 나무 6종, minmax·hidden 통합) · **최종 수정일:** 2026-08-17
> **기준 마스터:** v1.9.0 (§5.2 클라↔서버 복본 동기화 · §6.7 정답 은닉 규약 · §1 연령 스테이지)
...
| 버전 | 날짜 | 핵심 |
|---|---|---|
| **v0.11.2** | **08-17** | **공용 헤더 통일(Claude Code).** 퀴즈 랜딩만 쓰던 자체 `.topbar`(브랜드 + ← 홈으로)를 폐기하고 공용 헤더(`nav.css` + 인라인 마크업)로 교체 — 이제 랜딩에서 다른 페이지로 바로 이동할 수 있고 계정 아이콘이 로그인 상태를 표시한다(게이트 없음, 퀴즈는 무로그인 원칙). 헤더 전역: ‘내 자료’를 우측 액션에서 **메뉴 마지막 항목**(문제지 생성 옆)으로 옮기고 **모바일 아이콘 축약 폐지**. ⚠ 카드 아이콘 `.glyph` 가 헤더 브랜드 글리프와 이름이 겹쳐 `.card-top .glyph` 로 한정. 공용 7페이지 헤더 높이 64.7px 동일 실측. |
...
- **헤더 = 공용 헤더(v0.11.2).** 자체 `.topbar`(브랜드 + '← 홈으로')를 버리고 `assets/css/nav.css` + 인라인 마크업(정본 = `account/index.html`)으로 통일했다. 이제 퀴즈 랜딩에서도 다른 페이지로 바로 갈 수 있고 계정 아이콘이 로그인 상태를 보여준다(`auth.js` — **게이트는 걸지 않는다**, 퀴즈는 무로그인이 원칙).
  - ⚠ 이 페이지는 카드 아이콘에 `.glyph` 를 쓰고 있어 공용 헤더의 `.site-brand .glyph` 와 이름이 겹친다 → 카드 쪽을 `.card-top .glyph` 로 한정했다.
