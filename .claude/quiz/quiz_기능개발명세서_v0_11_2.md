...
> **버전:** v0.12.2 · **상태:** 초안 — 본문 = 현행 **19유형**(계산 3 + 위앞옆 3 + G군 3 + A군 6 + H군 4) · **최종 수정일:** 2026-08-21
...
- **1행 필터:** 교육과정(전체/교과/사고력) · **학년** · 난이도(다중 칩, 기본 중·상·최상) · **차원(전체/3D만/2D만)** · 문제 수(10/20/30).
- **학년(연령 스테이지) — 단일 출처는 서버 `/config`(v0.12.0).** 칩을 HTML 에 박아 두지 않고 열린 스테이지로 그린다. 닫힌 학년으로 들어온 링크(`?stage=S5` 등)는 기본값(초6)으로 되돌린다.
  - 칩 아래 **한 줄 설명**을 붙인다(`STAGES[].note` — 서버가 준다). "고르고 나서야 무슨 학년인지 알게 되는" 것을 막는 자리다.
  - **카드 격자와 카테고리 아코디언 둘 다** 같은 `gateFor()` 로 거른다. ⚠ 한쪽만 걸면 그 학년에 없는 유형이 목록에서 눌린다(유아에게 '안 보이는 나무'가 보였다 — v0.12.0 🐛).
  - 폴백표(`STAGE_FB`·`GATE_FB`)는 `/config` 가 못 올 때만 쓴다. **서버와 자동 대조**로 학년·라벨·설명·유형·서브 일치를 확인한다(눈검사 금지 — 마스터 §5.2 정신).
- **예시 미리보기 (v0.12.0)** — 카드의 [예시 보기] · 목록 행의 [예시].
  - **실제 `/generate` 문항 3개**를 모달로 보여준다. 현재 고른 학년·난이도가 그대로 간다. 손으로 만든 예시는 문구·난이도·제시물이 반드시 생성기와 어긋나므로 쓰지 않는다.
  - **정답은 응답에 없다.** [정답 보기]를 누르면 `answer:null` 로 `/grade` 를 호출해 `answerKey`·`explain` 을 받는다 → §3.12 은닉 규약이 그대로 유지된다.
  - 그림은 **`figures.renderQuestion(q)`** — 제시물(`given`) 또는 겨냥도(`sh`+`top`) + `facesMc` 보기까지 한 덩어리로 그린다. 퀴즈 화면·문제지와 **같은 렌더러**다(§2.4). 정답 표기 `figures.answerText(key,unit)` 도 공용으로 승격해 run·worksheets·미리보기 세 곳이 위임한다.
  - 정답 그림: `drawSil` 은 **물어본 방향(`views`)만** 칠한다(세 칸을 다 그리면 안 물어본 면을 알려준다) · `markCount` 는 위에서 본 수 격자 · `markCells`(A-b)는 위모양에 숨은 칸을 초록 표시(좌표 나열 금지).
  - ⚠ 3D 회전 6종은 미리보기에서 **정적 겨냥도**로 보여주고 "실제 퀴즈에서는 돌려볼 수 있다"고 안내한다(랜딩에 Three.js 를 싣지 않는다).
  - ⚠ 연타 대비 **요청 토큰**으로 늦게 온 응답을 버린다. 안 그러면 이미 바뀐 유형 위에 앞 요청 결과가 그려진다.
  - ⚠ 이 페이지는 `consent.js` 를 싣지 않아 `gtag` 가 없다. 그래도 **`track()` 정의는 있어야 한다** — 없으면 호출한 함수가 ReferenceError 로 통째로 죽는다(미리보기가 이것 때문에 열리지 않았다).
...
- 시작하기 → `./run/?type&sub&lv&n&dim&stage` (`sub`=서브타입, `stage`=연령 스테이지·기본값이면 생략).
- 필터 상태는 URL 쿼리로 동기화(`edu`·`dim`·`n`·`lv`·`stage`) → 공유 가능.
