...
## 개요
- **CubeNest(큐브네스트):** 초등 6학년 '쌓기나무(공간과 입체)' 3D 학습 웹 도구 — 3D 회전, 위·앞·옆 투상, 개수·부피·겉넓이 자동 계산, 블록 추가·삭제.
...
## 현재/다음
- **완료:** 상태 공유(F2·F3)·계측/동의(F1)·서버 생성/채점(gen 서버화)·OAuth 로그인.
- **다음:** DB+RLS 저장(quiz 결과·`/my`) → 결제(이용권) → 안보이는나무 6종·유형 리매핑(gen).
