...
- **직렬화(F2):** 채운 칸·격자·모서리 → URL-safe base64 `?m=`. **모양만 담고 문제 메타 금지.**
  ...
  - `&h=` 의 어떤 실패도 `?m=` 로드를 막지 않는다(무효→선택만 버림, toast 없음). 강조가 비면 `&h=` 를 아예 안 붙인다. `?h=` 만 있고 `?m=` 이 없으면 전면 무시. 검증은 `deserialize` 보다 **엄격**하다 — `&h=` 엔 버전 바이트가 없어 **길이가 유일한 교차검증 수단**이라 잘린 마스크를 관대하게 받으면 "부분적으로 맞는 색"이 조용히 통과한다.
