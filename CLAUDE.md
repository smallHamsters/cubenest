...
※ 상세 기획·불변 원리의 정본은 마스터 문서 `.claude/master/master.md`(v1.9.9, 260910). 코드 주석의 `§` 표기는 그 마스터의 장 번호를 가리킨다.

## 개요
- **CubeNest(큐브네스트):** '쌓기나무(공간과 입체)' 3D 학습 웹 도구 — 3D 회전, 위·앞·옆 투상, 개수·부피·겉넓이 자동 계산, 블록 추가·삭제.
- **대상: 만 5~11세**(한국 교육과정 기준, **기본 스테이지 = 초6** '공간과 입체'). 난이도는 **연령 스테이지가 유형·밴드를 먼저 정하고** 하/중/상/최상은 그 안의 상대 난이도다 — 출시 **S0~S4**(유아 · 초1~2 · 초3~4 · 초5 · **초6**(기본)), **S5(중1~2) 비노출**(설계는 코드에 보존). **S0·S1은 `count`·`facesMc` 2종만** 열고 `facesMc`는 묻는 방향 제한(S0=위 / S1=위·앞 / **S2=위·앞**). **학년별 열린 조합 = 2 / 2 / 5 / 7 / 19**(S2 = count·heightmap·facesMc·hidden A-c·A-e). 정본 = `.claude/quiz/cubenest_난이도_재설계_260818_v0_1.md` §7·§10.
...
- **모바일 메뉴 탭 타깃은 44px 이다**(`nav.css` `@media (max-width:640px)` — `min-height:44px` + 세로 가운데, 260906). 종전 29px(실제 히트 30px)라 **계정 버튼만 기준을 지키고 메뉴는 안 지키고 있었다.** **헤더 높이는 안 변한다** — 57px 를 이미 계정 버튼(44px)이 정하고 있어서 위아래 죽은 공간을 링크가 가져갈 뿐이다.
  - ⛔ **`::after` 로 히트 영역만 넓히는 흔한 손은 여기서 안 먹는다.** `.site-nav` 가 `overflow-x:auto` 라 사양상 **세로축의 `visible` 이 `auto` 로 바뀌어** 넓힌 영역이 잘린다(`overflow-y:visible` 로 되돌리려는 시도도 무시된다). **박스를 재면 45px 라 통과한 것처럼 보이는데 `elementFromPoint` 로 재면 30px 그대로다** — 탭 타깃은 박스가 아니라 **클릭 판정으로 확인할 것.**
  - 데스크톱(>640px)엔 넣지 않았다 — 마우스라 불필요하고 헤더만 두꺼워진다(현재 32px).
...
- **`scope.js` — 저장소 스코프·학습자의 단일 진실(`CubeNest.scope`, 260906 신설).** auth 를 참조하는 쪽이고 auth.js 는 이걸 모른다. 로드 순서 `auth.js → scope.js → mydata.js`(scope 5페이지 = quiz·quiz/run·worksheets·account·my / mydata 는 그중 4곳, quiz 랜딩 제외).
  ...
  - ⛔ **'학생 미지정'(`NONE='none'`)에 uuid 를 주지 말 것.** 로스터의 행이 아니라 **센티널**이다 — 원래부터 있던 "학습자 미선택" 상태에 이름과 자리를 준 것이고 **토큰은 빈 문자열**이라 키가 안 바뀐다. 실제 학습자로 만들어 기본 선택에 두는 순간 **전 사용자의 키가 `base` → `base~<새id>` 로 바뀌어** 진행 세션·`/my` 자료·닉네임이 통째로 사라진 것처럼 보인다. `learner()` 는 미지정에 **`null`** 을 돌려주고, 「골랐는가」는 `selection()`(`null`/`'none'`/uuid)이 따로 답한다 — `shouldAsk()` 가 `!curId()` 를 보면 **미지정을 고른 사람을 매번 다시 묻는다.**
    - 2단계 서버 모델과도 이게 맞다: **미지정 = `learner_id IS NULL`** 이라 `learners` 테이블에 행이 생기지 않는다.
  - `importDeviceLearners()` — 로그아웃 중 만든 학습자를 계정으로. 재키잉은 **키 이름을 열거하지 않고** `~<lid>` 로 끝나는 키를 `__<uid>~<lid>` 로 옮긴다(베이스가 늘어도 자동으로 따라온다). ⚠ **연습장은 `…~<lid>_sc` 라 접미어가 끝에 없다** — 꼬리를 떼고 봐야 한다(`splitScoped`). 빠뜨리면 진행은 따라오는데 **필기만 사라진다**(실제로 그렇게 났고 하네스가 잡았다).
...
- **헤더 메뉴에 '홈'은 없다**(260906 제거). `.site-brand`(로고+워드마크)가 이미 홈 링크이고(`aria-label="큐브네스트 홈"`) 로고→홈은 웹의 보편 관례라 순수 중복이었다. 모바일 가로 스크롤도 40px 만큼 덜 넘친다(**375px 실측: 넘침 128px → 88px**). ⚠ 다시 넣지 말 것.
  - ⚠ **모바일 넘침 자체는 안 풀린다.** 375px 에서 `.site-nav` 는 폭 241px 인데 내용이 329px 다(항목 폭 97·40·77·52·55). **480px 부터 5종이 다 들어온다** — 320:2/5 · 360~390:3/5 · 412~430:4/5. 즉 **휴대폰 세로는 전부 해당**이고 태블릿·데스크톱은 무관하다.
    ...
    - **완전히 닫으려면 라벨 축약(실측 88→17px)이나 햄버거뿐이고 둘 다 제품 결정이다** — `nav.css` 가 '숨기면 내비게이션이 사라진다'며 햄버거를 기각해 뒀고, 라벨은 랜딩 CTA·마스터 사이트맵과 함께 쓰는 제품 이름이다. 참고로 **글자·여백 축소는 88→50px 에 그치면서 탭 타깃만 낮춘다** — 기각.
...
- **정답 은닉 — 범위는 `/generate` 응답 스키마까지다.** `/generate` 응답에 **정답을 담지 않는다**(예전엔 facesMc 정답 번호·minmax rc·A-f kinds가 그대로 나갔다). 채점·색칠·해설의 단일 출처는 **`/grade` 응답의 `answerKey`·`explain`**. 로컬 폴백 채점 없음 → 실패 시 재시도(어차피 `/generate` 없이는 시작도 못 한다).
  ...
  - 이 동작에 `/quiz` 랜딩의 '정답 보기' 미리보기(`quiz/index.html` 의 `pvToggleAnswer()` — `answer:null` 로 `/grade` 를 부른다)가 의존한다 — 닫으려면 그 기능의 제품 결정이 선행된다.
...
- **배포는 두 갈래이고 순서가 있다 — 클라 먼저, 서버 나중.** 정적 사이트 = `git push` → GitHub Pages(origin/main). 서버 = `supabase functions deploy config generate grade worksheet`. 둘은 따로 나가므로 **한쪽만 배포하면 조용히 어긋난다** — 오류가 아니라 "안 바뀐 것처럼" 보인다.
  ...
  - ⛔ **게이트(`GATE`)를 좁히면 진행 중 세션이 조용히 오채점된다.** 닫힌 유형은 `support()` 가 `[]` 를 주고,
    `genSession` 이 등급 풀을 `['중']` 하나로 좁혀 **다른 문항이 재생성**된다. 그런데 `gsig` 는 *요청한* levels 를
    해시하므로 **검증은 통과한다** — 403 이 아니라 오채점이다(260910 `minmax G-c`@S2 로 로컬 재현: 6문항 중 4~6번이 달라졌다).
    노출은 그 (유형·학년)으로 **진행 중이던** 세션뿐이라 베타에선 수용했지만, 트래픽이 생기면 배포 시각을 고르거나 유예를 둔다.
  - ⚠ **게이트를 바꾸면 서버가 먼저다**(표준 순서의 예외). `bake-samples.mjs` 가 **배포된 `/config` 에서 조합을 읽고
    문제도 배포된 서버가 만들기 때문**이다 — 서버가 옛 상태면 표본이 옛 규칙으로 구워져 카드가 거짓말을 한다.
    순서: 서버 배포 → `/config` 캐시 300초 → 재굽기 → `check-drift.py` → 클라 push.
