...
- **핵심 자산은 서버(Edge Function):** `gen`·`gen-config`(전체 문제 공간·정답 규칙)는 클라에서 삭제됨. 클라는 **`quiz/run/api-client.js`**(`assets/js/` 아니다)로 `/generate`·`/grade` 호출. **`/grade`는 gsig(HMAC)로 위·변조 방지**, rate limit(익명 쿠키+IP). 무료 익명 플레이라 `verify_jwt=false`.
