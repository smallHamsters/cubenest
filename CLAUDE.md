...
- **대상: 만 8~13세**(한국 교육과정 기준, **기본 스테이지 = 초6** '공간과 입체'). 난이도는 **연령 스테이지가 유형·밴드를 먼저 정하고** 하/중/상/최상은 그 안의 상대 난이도다 — 출시 **S2~S5**(초3~4 · 초5 · **초6**(기본) · 중1~2), **S1(초1~2) 보류**. 정본 = `.claude/quiz/cubenest_난이도_재설계_260818_v0_1.md` §7.
...
## 현재/다음
- **완료:** 상태 공유(F2·F3)·계측/동의(F1)·서버 생성/채점(gen 서버화)·OAuth 로그인·**유형 19종**(리매핑 + 안보이는나무 A-a~f + G군 + H군)·정답 은닉·worksheets(문제지·정답지 인쇄).
- **다음:** **난이도 개편(연령 스테이지 + 개수 밴드, gen-config 구조 교체)** → DB+RLS 저장(quiz 결과·`/my`) → 결제(이용권).
