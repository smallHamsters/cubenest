...
- **정답 은닉 — 범위는 `/generate` 응답 스키마까지다.** `/generate` 응답에 **정답을 담지 않는다**(예전엔 facesMc 정답 번호·minmax rc·A-f kinds가 그대로 나갔다). 채점·색칠·해설의 단일 출처는 **`/grade` 응답의 `answerKey`·`explain`**. 로컬 폴백 채점 없음 → 실패 시 재시도(어차피 `/generate` 없이는 시작도 못 한다).
  ...
  - A-a/b/f의 답은 **보이는 그림만의 함수**다(숨은 열의 실제 높이는 어느 답에도 안 쓰임). 그래서 그림을 주는 것 자체는 정답을 흘리지 않는다.
    - ⚠ **다만 "치터도 학생처럼 그림을 분석해야 한다"는 성립하지 않는다**(260830 확인, 미수정). `cubenest-iso.js:66-79` 가 `sh.cells` **전부**를 돌며 큐브당 폴리곤 3개를 방출하고, 가림은 오클루전 컬링이 아니라 **페인터 정렬로 덮어 그리는 것**뿐이다(`:44`). 즉 **화면상의 은닉이지 페이로드상의 은닉이 아니다** — `<polygon` 개수로 나무 수가 바로 나오고(`(개수−1)/3`), `PT()`(`:47`)가 결정적이라 좌표 역산으로 형상까지 복원된다. 진짜 은닉을 원하면 오클루전 컬링이 필요하고, 그때 **클라 정본(`assets/js/`)과 `_shared/` 복본을 동시에** 고쳐야 한다.
