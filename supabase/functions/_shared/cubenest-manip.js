 ...
 *   [정본] 서버 단일본 (마스터 §5.2 규칙 4 · 260821).
 *     클라 사본 assets/js/cubenest-manip.js 는 어떤 페이지도 로드하지 않는 죽은 사본이었고,
 *     그 사이 여기에만 H-d 직육면체 일반화(paintedBox*·solidBoxHmap, 260819)가 들어가
 *     증상 없이 갈라져 있었다 — 사본을 삭제해 갈래 자체를 없앴다(§5.2 드리프트 실증 건).
 *     UMD 형태는 유지 — 클라가 다시 필요해지면 이 파일을 assets/js/ 로 올려 정본으로 삼는다.
 *   좌표: 위=+y, 앞=+z, 옆=+x. heightMap = {"x,z": 높이}.
 * ==========================================================================*/
(function (global) {
  'use strict';

  var VERSION = '0.2.0';   // 0.2.0 = H-d 직육면체 일반화 흡수 + 서버 단일본화(§5.2)
...
export const manip = globalThis.CubeNest.manip;
