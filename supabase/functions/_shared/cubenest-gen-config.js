 ...
 * ==========================================================================*/
(function (global) {
  var VERSION = "cfg-0.5.0";   // 0.5.0 = GATE.S2 교체(minmax G-c 제거 · facesMc 개방 + dirs 제한)
  ...
  //   ⚠ 이건 **현행 교과서 관례**다 — 2022 개정 교육과정은 학년군(1~2/3~4/5~6)으로
  //     성취기준을 주고 학기 배치는 교과서별이다(§7.1 원주). 문서의 이 단서가 코드로
  //     옮겨지면서 단정으로 굳었길래 되살린다. 게이트를 옮길 땐 이 폭을 기억할 것.
  ...
  var GATE = {
    ...
    // 초3~4 — ⚠ 여기에 유형을 열 때는 **제시물을 직접 확인**할 것. STAGES[].dim 은
    //   hidden·minmax·manip·facesDraw 를 못 막는다(GIVEN_ONLY 가 resolveDim 에서 먼저 반환).
    //   260910 에 minmax G-c 를 뺐다: §7.4 의 근거가 "세기 과제라 저학년도 가능"이었는데
    //   **제시물을 계상하지 않았다.** 답 countAtLeast(hmap,n) 은 n층 판의 칸 수라
    //   층별·위에서 본 수로 주면 답이 그대로 읽힌다 → 부담이 통째로 '겨냥도 읽기'다(=초5).
    //   같은 날 facesMc 를 열었다: S0·S1·S3 은 열려 있는데 S2 만 닫히고 사유가 없었다(오탈락).
    S2: { count: true, heightmap: true, facesMc: true, hidden: ["A-c", "A-e"] },
  ...
  var TYPE_AXES = {
    ...
    //   S2(초3~4)도 위·앞까지다(260910) — 위·앞·옆 전부는 §7.4 가 "겨냥도를 배운 직후"라며
    //   S3 에 준 것이라, 지정을 빼면 초3~4 가 삼면도를 먼저 받는다. S1 과 방향은 같고 BANDS 로 커진다.
    facesMc: {
      dirs: { S0: ["top"], S1: ["top", "front"], S2: ["top", "front"] }
    },
